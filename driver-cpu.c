...
extern bool ScanHash_4WaySSE2(struct thr_info*,
    struct work* work,
    uint8_t* max_nonce,
    uint8_t* last_nonce,
    uint8_t* nonce,
...
extern bool ScanHash_altivec_4way(struct thr_info*,
    struct work* work,
    uint8_t* max_nonce,
    uint8_t* last_nonce,
    uint8_t* nonce,
...
extern bool scanhash_via(struct thr_info*,
    struct work* work,
    uint8_t* max_nonce,
    uint8_t* last_nonce,
    uint8_t* nonce,
...
extern bool scanhash_c(struct thr_info*,
    struct work* work,
    uint8_t* max_nonce,
    uint8_t* last_nonce,
    uint8_t* nonce,
...
extern bool scanhash_cryptopp(struct thr_info*,
    struct work* work,
    uint8_t* max_nonce,
    uint8_t* last_nonce,
    uint8_t* nonce,
...
extern bool scanhash_asm32(struct thr_info*,
    struct work* work,
    uint8_t* max_nonce,
    uint8_t* last_nonce,
    uint8_t* nonce,
...
extern bool scanhash_sse2_64(struct thr_info*,
    struct work* work,
    uint8_t* max_nonce,
    uint8_t* last_nonce,
    uint8_t* nonce,
...
extern bool scanhash_sse4_64(struct thr_info*,
    struct work* work,
    uint8_t* max_nonce,
    uint8_t* last_nonce,
    uint8_t* nonce,
...
extern bool scanhash_sse2_32(struct thr_info*,
    struct work* work,
    uint8_t* max_nonce,
    uint8_t* last_nonce,
    uint8_t* nonce,
...
double bench_algo_stage3(enum sha256_algos algo)
{
    ...
    gettimeofday(&start, 0);
    {
        ...
        (*func)(&dummy, &work, max_nonce, last_nonce, &hashes_done);
    }
    gettimeofday(&end, 0);
...
static void set_non_blocking(int fd, int yes)
{
...
static double bench_algo_stage2(enum sha256_algos algo)
{
    ...
    char unique_name[32];
    snprintf(unique_name, sizeof(unique_name) - 1, "cgminer-%p", (void*)module);
    ...
    BOOL ok = CreateProcess(NULL, // No module name (use command line)
    ...
    while (1)
    {
        ...
        int go_on = CREATE_PROCESS_DEBUG_EVENT == debug_event.dwDebugEventCode ||
        ...
        {
            int go_on = EXCEPTION_BREAKPOINT == debug_event.u.Exception.ExceptionRecord.ExceptionCode;
            if (!go_on)
                break;
        }
...
static void bench_algo(double* best_rate, enum sha256_algos* best_algo, enum sha256_algos algo)
{
...
static enum sha256_algos pick_fastest_algo()
{
    ...
    memset(name_spaces_pad, ' ', n);
    name_spaces_pad[n] = 0;
    applog(LOG_ERR, "\"%s\"%s : is fastest algorithm at %.5f MH/s", algo_names[best_algo], name_spaces_pad, best_rate);
    return best_algo;
}
...
// Reckon number of cores in the box
#if defined(WIN32)
    {
        DWORD system_am;
        DWORD process_am;
        BOOL ok = GetProcessAffinityMask(GetCurrentProcess(), &system_am, &process_am);
        if (!ok)
        {
    ...
    if (unlikely(!cpus))
        nxquit(1, "Failed to calloc cpus");
...
static void cpu_can_limit_work(struct thr_info __maybe_unused* thr, uint8_t* max_nonce)
{
    memcpy(max_nonce, MAX_POSSIBLE_NONCE, 16);
}
...
static void incrementNonce(uint8_t nonce[16])
{
    int i = 0;
...
static uint64_t cpu_scanhash(struct thr_info* thr, struct work* work, uint8_t max_nonce[16])
{
    const int thr_id = thr->id;
    uint8_t mining_nonce[16];
    memcpy(mining_nonce, work->nonce, 16); // start nonce for the thread into first_nonce
    bool rc;
    uint64_t hashes_done = 0;
CPUSearch:
    rc = false;
    ...
    /* scan nonces for a proof-of-work hash */
    {
        ...
        rc = (*func)(thr, work, max_nonce, mining_nonce, &hashes_done);
    }
    ...
    if (unlikely(rc))
    {
        ...
        memcpy(work->nonce, mining_nonce, 16);
        // store nonce in device nonce in case we are restarting work due to some change
        // that was not a new block detected on the network
