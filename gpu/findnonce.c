...
const uint32_t SHA256_K[64] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4,
...
void precalc_hash(dev_blk_ctx* blk, uint32_t* state, uint32_t* data)
{
    ...
    blk->PreVal4 = blk->fcty_e =
        blk->ctx_e + (rotr(B, 6) ^ rotr(B, 11) ^ rotr(B, 25)) + (D ^ (B & (C ^ D))) + 0xe9b5dba5;
...
#define P(t)                                                                                                   \
    (W[(t)&0xF] = W[(t - 16) & 0xF] +                                                                          \
                  (rotate(W[(t - 15) & 0xF], 25) ^ rotate(W[(t - 15) & 0xF], 14) ^ (W[(t - 15) & 0xF] >> 3)) + \
                  W[(t - 7) & 0xF] +                                                                           \
                  (rotate(W[(t - 2) & 0xF], 15) ^ rotate(W[(t - 2) & 0xF], 13) ^ (W[(t - 2) & 0xF] >> 10)))
